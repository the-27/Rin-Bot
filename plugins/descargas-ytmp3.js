...
const sendAudioWithRetry = async (conn, chat, audioUrl, videoTitle, maxRetries = 2) => {
  ...
  let thumbnailBuffer;
  try {
    const response = await axios.get('https://files.catbox.moe/c8ynqc.jpg', { responseType: 'arraybuffer' });
...
let handler = async (m, { conn, text }) => {
  ...
  const reactionMessage = await conn.reply(m.chat, `*${emojis} Descargando su audio, Espere un momento...*`, m);
  ...
  try {
    ...
    await conn.reply(m.chat, `😼 *Error:* ${error.message || "Desconocido"}`, m);
  }
};

handler.help = ['mp3 <url de youtube>'];
